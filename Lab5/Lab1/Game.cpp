...
/// </summary>
Game::Game() :
	m_window{ sf::VideoMode{ sf::Vector2u{800U, 800U}, 32U }, "SFML Game 3.0" },
...
sf::Vector2f Game::getFormationTarget(sf::Vector2f t_offset, float t_angle)
{
	sf::Vector2f target;
	float angleRad = (t_angle) * M_PI / 180;

	target.x = t_offset.x * std::cos(angleRad) - t_offset.y * std::sin(angleRad);
...
void Game::setupTexts()
{
	...
	m_cursor.setRadius(2.0f);
	m_cursor.setFillColor(sf::Color::Red);
	m_cursor.setOrigin(sf::Vector2f(2.0f, 2.0f));
	m_cursor.setPosition(sf::Vector2f(100.0f, 100.0f));
}
